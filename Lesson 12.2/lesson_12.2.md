# Studies Assistant Web App: "How to create a Notepad":
...
- [Notepad Script's Codepen Example](https://codepen.io/cromerolo2/pen/JjzBzyw)
...
With that said, then let's dive in!

In this lesson we will focus mostly in editing the contents of the element with class "accordion-item".

Locate in your `index.html` file, the following snippet of HTML: 
...
### Steps (continuation from Lesson 12.1)

#### Step 12

Inside of the `<section>` with class "accordion-body".

Let's create a `<form>` tag, with the `style` attribute value equals to "padding: 5px;"
...
For this, create a `<button>` tag with the following attributes:
...
[document.execCommand() is deprecated](https://developer.mozilla.org/en-US/docs/Web/API/Document/execCommand) and might not be supported in your browser in the future.
...
```
The examples for this lesson are now finished. (But you still have work to do on your own xD) 

Your next tasks regarding the Notepad are:

1. Write the required code for the "feature" that will allow users to download contents of the Notepad as a file .
    - call the function you created to download contents of Notepad as a file using the `onclick` attribute of `<li><a class="dropdown-item" href="#">Download file</a></li>`.
2. Create a button `<button>` or link `<a>`for clearing Browser's localStorage (make use of the example code for `let cleanLocalStorage` as the `onclick` attribute for the html tag).

After completing your tasks, you should open the `index.html` file in the Browser and test the notepad (fix your code if necessary).

Other useful features that you could implement (not in the provided code) are:

- Support for Ordered and Unordered Lists style. (Provide a button like the ones for Italic, Bold, etc) 
- Configurable text color. (What about highliting?)
- Configurable fonts and text size.
- Support for including Images.
- Letting the user to work on multiple "documents" without having to download a copy to their computer disk when they desire to work on other things. (Implement logic for your own "File Handling System" with keys and values persisted in the Browser's LocalStorage)
- Voice-to-Text implementation with "configurable" voice commands.
- Communication with an online LLM. 

---

Below, following the spirit of the lesson 6.2, you can find the JavaScript code for a "WYSIWYG" text editor that runs in the browser.

You'll notice tha re-writing the Notepad as an "unnecessarily verbose" JavaScript piece of code, uses many more lines to achieve the same UI look, but in this journey of learning I think it may help you to better understand the topics like DOM Manipulation, LocalStorage, etc... (Some could argue that the current studies app HTML can be created by adding a few more lines to the following script).

In case you decide to use the code below, maybe consider JavaScript Modules to make it more mantainable. 

(Because it's written as an inline script) It's recommended that you read it the from the bottom up, starting by:

```javascript

//Higher level function for Notepad component
...
// Call function createNotepadApp to create Notepad app.
createNotepadApp();
...
*/
const NOTEPAD_PARENT_ELEMENT_ID = "collapseThreeAccordionBody";
...
// Call createNotepadApp to create Notepad app.
createNotepadApp();

``` 


This tutorial is part of a series, please continue in the [Next lesson](https://github.com/CristianRomero1234/web-development-course/tree/main/Lesson%2012%2E3/lesson_12%2E3.md)